# Melih Küçük

Computer Engineering student at Dokuz Eylül University.

I develop console applications with Java and C#. My projects focus on data structures, algorithms, object-oriented programming, Boolean logic, and file operations.

## Individual Projects

- [Race Tournament Simulator](https://github.com/melih-kck/CME1212-Race-Tournament-Simulator) — Java tournament game built with custom single and double linked lists.
- [Alphabet Match Game](https://github.com/melih-kck/CME1212-Alphabet-Match-Game) — Java matching game built with custom stack and queue implementations.
- [Mini Excel](https://github.com/melih-kck/CME1211-Mini-Excel) — C# console spreadsheet with arithmetic, text processing, encryption, and file operations.
- [Coffee Shop Game](https://github.com/melih-kck/CME1211-Coffee-Shop-Game) — C# management game with inventory, budget, order, and daily operations.

## Team Projects

- [Snowball Game](https://github.com/melih-kck/CME1251-Snowball-Game) — C# two-player console game with projectile movement, wind, obstacles, and turn-based gameplay.
- [Logic Maze](https://github.com/melih-kck/CME1252-Logic-Maze) — Java logic game featuring expression trees, truth tables, and Karnaugh maps.
- [Twins](https://github.com/edakirci/Twins1252) — Java maze game focused on treasure collection, scoring, and enemy movement.
- [Squares](https://github.com/elffkse/puzzleGame) — C# puzzle game developed as a team project.

## Technologies

- Java
- C# and .NET
- Git and GitHub

## Profiles

- [LinkedIn](https://www.linkedin.com/in/melihkucukdev/)
- [ORCID](https://orcid.org/0009-0005-2013-9661)
